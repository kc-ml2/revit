"""REViT windowed backbone for downstream tasks (e.g. OBB detection).

Fork of ``revit_windowed_gcsa.py`` with detection-oriented helpers:
- optional ``num_classes=None`` (no ImageNet classification head)
- ``stage_types`` exposed for necks / group-pooling
- ``forward_features(..., return_stages=True)`` for multi-scale maps
"""
...
class Rot2DTransformerV2(nn.Module):
    def __init__(
        ...
        self.head = (
            Rot2DClassificationHeadV2(stage_types[-1], gspace, num_classes=num_classes, dropout=dropout)
    ...
    def forward_features(self, x: torch.Tensor, return_stages: bool = False):
        """Run stem + stages. If return_stages, yield each stage map before its downsample."""
        x = self.stem(x)
        feats = []
        ...
        return feats if return_stages else x

    def forward(self, x: torch.Tensor, return_probs: bool = False):
        x = self.forward_features(x, return_stages=False)
        if self.head is None:
            raise RuntimeError("Rot2DTransformerV2 was built without a classification head (num_classes=None)")
